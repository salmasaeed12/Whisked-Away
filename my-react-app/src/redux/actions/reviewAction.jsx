...
export const createReview = (prodID, body) => async (dispatch) => {
    try {
        ...
        dispatch({
            type: CREATE_REVIEW,
            payload: e.response?.data?.message || "Error creating review",
        });
    }
};
...
export const allReviewProduct = (prodID, page, limit) => async (dispatch) => {
    try {
        ...
        dispatch({
            type: ALL_REVIEW_PRODUCT,
            payload: e.response?.data?.message || "Error fetching reviews",
        });
    }
};

// Delete review for one product
export const deleteReviewOnProduct = (id) => async (dispatch) => {
    try {
        ...
        dispatch({
            type: DELETE_REVIEW,
            payload: e.response?.data?.message || "Error deleting review",
        });
    }
};

// Update review for one product
export const updateReviewOnProduct = (id, body) => async (dispatch) => {
    try {
        ...
        dispatch({
            type: UPDATE_REVIEW,
            payload: e.response?.data?.message || "Error updating review",
        });
    }
};